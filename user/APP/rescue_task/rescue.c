...
static void Rescue_control_PID(rescue_control_e *rescue_calc)
{
    ...
    for (i = 0; i <= 1; i++)
    {
        if (rescue_calc->Claw_mode[i] == OPEN)
        {
            ...
            rescue_calc->give_current[i]=PID_Calc(&rescue_calc->rescue_speed_pid[i], (fp32)rescue_calc->rescue_motor_measure[i]->speed_rpm, rescue_calc->rescue_count_pid[i].out);
        }
